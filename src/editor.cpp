...
{
#ifdef ZEP_CONSOLE
    spZep = std::make_shared<ImGui::ZepConsole>(Zep::ZepPath(appRoot));
#else
    // Initialize the editor and watch for changes
    spZep = std::make_shared<ZepWrapper>(appRoot, Zep::NVec2f(pixelScale.x, pixelScale.y), [](std::shared_ptr<ZepMessage> spMessage) -> void {
    });
#endif
...
{
    auto pBuffer = zep_get_editor().InitWithText(name, text);
}
